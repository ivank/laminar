...
import { document, Type, printDocument, withHeader, withImports } from '@ovotech/ts-compose';
...
export const toTypeScript = (schema: Schema, initial: Context = {}): string => {
  ...
  const name = ts.isTypeReferenceNode(type) && ts.isQualifiedName(type.typeName) ? type.typeName.right : 'AvroType';

  return printDocument(document(contextWithHeader, Type.Alias({ name, isExport: true, type })));
};

export const toExternalContext = (schema: Schema, initial: Context = {}): { [key: string]: Schema } => {
  const contextWithRefs = collectRefs(schema, initial);
