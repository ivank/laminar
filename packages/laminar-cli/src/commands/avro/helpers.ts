...
export const firstUpperCase = (name: string): string => (name ? name[0].toUpperCase() + name.slice(1) : name);

export const convertNamespace = (namespace: string): string => namespace.split('.').map(firstUpperCase).join('');

export const nameParts = (fullName: string): [string] | [string, string] => {
  const parts = fullName.split('.');
  return parts.length > 1 ? [parts.slice(parts.length - 1)[0], parts.slice(0, parts.length - 1).join('.')] : [parts[0]];
};
