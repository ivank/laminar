...
import { HttpService, LoggerLike, Middleware, run, router, post, jsonOk, get, jsonNotFound } from '@ovotech/laminar';
...
describe('Integration', () => {
  ...
  it('Should start and stop services', async () => {
    ...
    await run({ initOrder: [pool, http] }, async () => {
      ...
      expect(loggerMock.debug).toHaveBeenCalledTimes(400);
      expect(loggerMock.debug).toHaveBeenCalledWith('PG Client: Query', {
        query: 'INSERT INTO animals (name) VALUES ($1) RETURNING id',
        values: ['test-0'],
      });
      expect(loggerMock.debug).toHaveBeenCalledWith('PG Client: Result', {
        query: 'INSERT INTO animals (name) VALUES ($1) RETURNING id',
        milliseconds: expect.any(String),
