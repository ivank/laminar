...
export const messages: Messages = {
  ...
  dependencies: ({ name, param }) =>
    `[${name}] (${'dependencies'}) requires [${toArray(param)}] keys`,
  ...
  oneOf: ({ name, param }) =>
    Array.isArray(param)
      ? `[${name}] (${'oneOf'}) should satisfy exactly only 1 schema\n${padWith(
          '  | ',
          formatVariants(name, param),
        )}`
      : `[${name}] should match only 1 schema`,
  anyOf: ({ name }) => `[${name}] (${'anyOf'}) should match at least 1 schema`,
};
