...
export const table = (rows: string[][]): string => {
  ...
  const columnLengths = columns.map((column) => column.reduce((len, item) => Math.max(len, ansiLength(item)), 0));

  return rows
    .map((row) =>
      row.map((item, index) => item + ' '.repeat(columnLengths[index] - ansiLength(item))).join(chalk.gray(' | ')),
    )
    .join('\n');
};
